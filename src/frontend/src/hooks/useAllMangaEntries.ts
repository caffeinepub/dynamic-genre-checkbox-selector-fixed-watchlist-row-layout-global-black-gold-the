...
import { readMangaCache, writeMangaCache } from "../utils/offlineMangaCache";
import {
  readMangaIndexedDbCache,
  writeMangaIndexedDbCache,
} from "../utils/offlineMangaIndexedDbCache";
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (indexedDbCached && indexedDbCached.length > 0) {
          console.warn(
            "Using IndexedDB cached manga data due to fetch error:",
            error,
          );
          return indexedDbCached.map((entry) => ({
            ...entry,
            coverImages:
              (entry as any).coverImageUrls?.map((url: string) =>
        ...
        if (localStorageCached && localStorageCached.length > 0) {
          console.warn(
            "Using localStorage cached manga data due to fetch error:",
            error,
          );
