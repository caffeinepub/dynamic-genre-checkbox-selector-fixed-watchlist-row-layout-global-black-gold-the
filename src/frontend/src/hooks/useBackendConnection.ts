...
import { TimeoutError, withTimeout } from "../utils/promiseTimeout";
...
export function useBackendConnection(): BackendConnectionState {
  ...
  const elapsedTimeInterval = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  const [isPerformingSafeguardReset, setIsPerformingSafeguardReset] =
    useState(false);
  ...
  const connectionQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const isStale = () =>
        attemptSessionGeneration !== sessionGeneration.current;
            ...
            if (isAuthenticated) {
              try {
                ...
                console.warn(
                  ...
                  accessControlError?.message || accessControlError,
                );
    ...
    retry: (failureCount, error) => {
      ...
      if (
        classified.category === "connecting" &&
      ...
      const shouldRetry =
        classified.shouldRetry && classified.category !== "authorization";

      if (
        !shouldRetry &&
        retrySessionGeneration === sessionGeneration.current
      ) {
        setConnectionPhase("Connection failed");
        if (elapsedTimeInterval.current) {
          clearInterval(elapsedTimeInterval.current);
  ...
  const classified = connectionQuery.error
    ? classifyError(connectionQuery.error)
    : null;
  ...
  return {
    ...
    elapsedTime,
    connectionPhase: isPerformingSafeguardReset
