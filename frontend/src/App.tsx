...
function AppContent() {
  ...
  const { isConnecting, isFailed, errorMessage, retry } = useBackendConnectionSingleton();
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
          <div className="text-center space-y-4">
            ...
            <p style={{ color: '#8a6a10' }} className="text-sm">
              {isLoggingIn ? 'Logging in...' : 'Please log in to access your watchlist'}
            </p>
          </div>
        </div>
      </AppLayout>
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-sm text-center max-w-md" style={{ color: '#8a6a10' }}>
            {errorMessage || 'Failed to connect to the backend. Please try again.'}
          </p>
          <button
            onClick={retry}
