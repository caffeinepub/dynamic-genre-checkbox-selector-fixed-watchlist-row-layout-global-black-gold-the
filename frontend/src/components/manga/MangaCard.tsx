...
import { useToggleBookmark, useUpdateChapterProgress } from '../../hooks/useMangaMutations';
...
export default function MangaCard({ entry, width, alignment = 'left' }: MangaCardProps) {
  ...
  const coverUrl = useCachedCoverImageUrl(principal, entry.stableId, networkCoverUrl);
  ...
  const handleNotesMouseEnter = useCallback((e: React.MouseEvent) => {
    if (entry.notes?.trim()) {
      setNotesAnchorRect((e.currentTarget as HTMLElement).getBoundingClientRect());
  ...
  const handleChaptersBlur = useCallback(() => {
    const parsed = parseFloat(chaptersValue);
  ...
  }, [chaptersValue, entry.stableId, entry.availableChapters, updateChapterProgress]);
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ width: '48px', minHeight: '72px' }}
        ...
        <div className="flex-1 min-w-0 flex items-center px-3 gap-3">
          ...
          <div className="flex-shrink-0 w-28 text-center">
            {editingChapters ? (
              ...
              <button
                ...
                style={{ color: '#d4a017', background: 'none', border: 'none', cursor: 'pointer' }}
          ...
          <div className="flex-shrink-0 w-32 hidden md:flex flex-wrap gap-1">
            ...
            {entry.genres.length > 2 && (
              <span className="text-xs" style={{ color: '#8a6a10', fontSize: '10px' }}>
                +{entry.genres.length - 2}
              </span>
            )}
          </div>
        ...
        <div className="flex-shrink-0 flex items-center px-2">
          ...
          >
            {entry.isBookmarked ? (
              <BookmarkCheck size={16} style={{ color: '#d4a017', fill: '#d4a017' }} />
            ) : (
